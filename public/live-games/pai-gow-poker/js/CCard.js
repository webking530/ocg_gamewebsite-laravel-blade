function CCard(iX,iY,oContainer){
    ...
    this._init = function(iX,iY,oContainer){
        ...
        _oCardSprite.on("click",this._onCardRelease);
    ...
    this.unload = function(){
        _oCardSprite.off("click",this._onCardRelease);
    ...
    this.setInfo = function(pStartingPoint,pEndingPoint,szFotogram,iValue,iSuit,bDealerCard,iCountCard){
        _bRemoving=false;
        _iTimeElaps=0;
    ...
    this.initSplit = function(iEndX,iEndY,iDepth){
        oContainer.setChildIndex ( _oCardSprite , iDepth );
        _pStartingPoint=new CVector2(_oCardSprite.x,_oCardSprite.y);
        _pEndingPoint=new CVector2(iEndX,iEndY);
        _iTimeElaps=0;
        _iState=STATE_CARD_SPLITTING;
    };
    ...
    this._onCardRelease = function(){
        ...
        if(_iState === STATE_CARD_WAIT_USER){
            _bSelected = !_bSelected;
            if(_bSelected){
                createjs.Tween.get(_oCardSprite).to({y:_oCardSprite.y - 20}, 300);
            }else{
                createjs.Tween.get(_oCardSprite).to({y:_oCardSprite.y + 20}, 300);
            }
        }
        
        s_oGame.cardSelected(_bSelected);
    };
    ...
    this.getChildIndex = function(){
        return _oContainer.getChildIndex(_oCardSprite);
    };
    ...
    this._updateSplit = function(){
        _iTimeElaps+=s_iTimeElaps;
        if (_iTimeElaps > TIME_CARD_SPLIT) {
            _iTimeElaps = 0;
            if(_aCbCompleted[SPLIT_CARD_END_ANIM]){
                _aCbCompleted[SPLIT_CARD_END_ANIM].call(_aCbOwner[SPLIT_CARD_END_ANIM],this,_iCountCard);
            }
            _iState=-1;
        }else{
            var fLerp = easeInOutCubic( _iTimeElaps, 0, 1, TIME_CARD_SPLIT);
            var oPoint = new CVector2();
